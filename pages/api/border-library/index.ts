...
export default createApiHandler({
  GET: async (req: NextApiRequest, res: NextApiResponse) => {
    try {
      ...
      if (error) {
        ...
        return res.status(500).json({ error: "Failed to fetch borders" });
      }

      return res.status(200).json(data);
    } catch (error) {
      console.error("Border library GET error:", error);
  ...
  POST: async (req: NextApiRequest, res: NextApiResponse) => {
    try {
      ...
      if (error) {
        ...
        return res.status(500).json({ error: "Failed to create border" });
      }

      return res.status(201).json(data);
    } catch (error) {
      console.error("Border library POST error:", error);
      return res.status(500).json({ error: "Internal server error" });
    }
  },
  ...
  DELETE: async (req: NextApiRequest, res: NextApiResponse) => {
    try {
      ...
      if (!id) {
        return res.status(400).json({ error: "Border ID is required" });
      }
      ...
      if (error) {
        ...
        return res.status(500).json({ error: "Failed to delete border" });
      }

      return res.status(200).json({ message: "Border deleted successfully" });
    } catch (error) {
      console.error("Border library DELETE error:", error);
      return res.status(500).json({ error: "Internal server error" });
    }
  },
});
