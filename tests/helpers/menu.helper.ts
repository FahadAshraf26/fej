...
const sortAssetsImages = async () => {
  const { data: globalTemplates, error: globalTemplatesError } = await supabase
    .from("assets")
    .select("id, createdBy, content ,restaurant_id (id, name), height, width")
...
export const getElementsWithRestaurant = async (user: IUserDetails | null, templateId?: number) => {
  ...
  const elementList = elements?.data?.map((item: any, i) => {
    ...
    }/storage/v1/object/public/elementsThumbnail/${item.thumbnail}?${i}${Date.now()}`;
  ...
  const libraryElements =
    user?.role == "flapjack"
      ...
      : elementList?.filter((item: any) => item?.restaurant_id?.toString() === user?.restaurant_id);
  ...
  const ElementsSectionList =
    user?.role == "flapjack"
      ...
      : [];
  const globalTemplates = user?.role == "flapjack" ? await sortAssetsImages() : [];
