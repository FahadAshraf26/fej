...
export const useCustomModalFont = ({
  setIsUploadFontModal,
  input,
  fontListTrigger,
}: {
  setIsUploadFontModal: (value: boolean) => any;
  input: number;
  ...
  useEffect(() => {
    const removeDelayedItems = () => {
      var elementWithShadowRoot = document.querySelector(
        ".CESDKModal_cesdkContainer__0rmWt #root-shadow "
      );
      ...
      const existingHeader = fontList.querySelector(".font-modal-header");
      if (existingHeader) {
        return;
      }
      ...
      headerContainer.className = "font-modal-header";
