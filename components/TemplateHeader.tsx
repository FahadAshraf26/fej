...
import { IconChevronDown, IconDownload, IconLogout, IconMail, IconSettings } from "@tabler/icons";

import { useSessionContext, useSupabaseClient } from "@supabase/auth-helpers-react";
import { useDialog, useUser, useUpsell, canCreateTemplate } from "../hooks";
...
const TemplateHeader = ({
  ...
  const activeClassFun = (value: string) => {
    navMenu && setNavMenu && setNavMenu(value);
          ...
          <Flex sx={{ marginLeft: "2rem" }}>
            ...
            {(user?.role == "user" && user?.subscriptionActive) ||
              (user?.role === "owner" || (user?.role === "user" && !!user?.restaurant_id) ? (
                ...
                <Text
                  className={`templates ${navMenu === "templates" ? "active" : ""} cursor-pointer`}
                  fz="sm"
                  ml="sm"
                  onClick={() => {
            ...
            {user?.role === "flapjack" && (
              <>
                <Text
                  // navMenu "cursor-pointer"
                  className={`myMenu ${navMenu === "customerMenus" ? "active" : ""} cursor-pointer`}
                  ml="sm"
                  fz="sm"
                  onClick={() => {
        ...
        <Flex align="center">
          {router.pathname.includes("templates") ? (
            ...
            <>
              <Button
                size="xs"
                variant="subtle"
                onClick={session ? triggerUpsellOr(onTemplateDownload) : openAuthDialog}
                sx={{
                  "&:hover": {
          ...
          {onTemplateSaveUpdate && (
            <Button
              size="xs"
              color="orange"
              onClick={session ? triggerUpsellOr(onTemplateSaveUpdate) : openAuthDialog}
              sx={{ marginRight: "1rem" }}
            >
              {router.query.id
                ? user?.role === "flapjack"
